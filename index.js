...
client.on('message', message => {
  if(message.content === 'как прошел день?') {
    message.reply('да как обычно, с карателями из Правого Сектора идем после кровавой операции по зачистке детского садика, было съедено 32 ребенка и 2 воспитателя. @вишня#4212');
    message.channel.send({files: ["./images/praviysektor.gif"]})   
  }
})
...
client.on('message', message => {
  if(message.content === 'Здрасьте') {
    message.reply('у холодноярців заведено замість «Здоров» вітатися «Слава Україні», а відповідається — «Україні слава»')
  }
})